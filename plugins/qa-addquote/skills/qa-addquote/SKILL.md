...
Feature files live at: `src/test/resources/features/provider/{ProviderFolder}/{Provider}_Quote.feature` (quote tests) or `src/test/resources/features/provider/{ProviderFolder}/{Provider}_Catalogue.feature` (catalogue tests).
...
- `NOS` → `NOS/NOS_Quote.feature` — EAccess/IpService; uses inline address columns

Catalogue tests:
- `NOS` → `NOS/NOS_Catalogue.feature` — validates EPL/EVPL, DIA, Broadband, Premium Internet products

---

### UAP Step 0: Test Category

Ask the user using AskUserQuestion:

**Question** (header: "Test Category")
- Question: "What type of test are you adding?"
- Options:
  - "Quote test" (description: "End-to-end quote integration test — submits a requirement and validates pricing response")
  - "Catalogue test" (description: "Catalogue validation test — verifies supplier product catalogue content (products, speeds, terms, media)")

**If "Catalogue test":** skip to [UAP Catalogue Workflow](#uap-catalogue-workflow).
**If "Quote test":** continue to UAP Step 1 below.
...
```
src/test/resources/features/provider/{ProviderFolder}/{Provider}_Quote.feature
```
...
- **File**: `src/test/resources/features/provider/{Provider}/{Provider}_Quote.feature` (NEW if created)
- **Mode**: UAP
...
## UAP Catalogue Workflow

Catalogue tests validate that the supplier product catalogue registered in UAP matches the expected specification (products, speeds, terms, and media). These are separate from quote tests and live in `{Provider}_Catalogue.feature` files.

Catalogue endpoint: `GET /catalogue/supplier/{supplierName}` (via `catalogueServiceUrl`)

Validation template: `classpath:helpers/catalogue_validation_template.feature` — validates `productName`, `requirementType`, `countryCodes`, `terms`, `media`, and `speeds` bidirectionally (missing and unexpected extras both fail).

### Catalogue Step 1: Existing or New Provider

Ask using AskUserQuestion:

**Question** (header: "Provider")
- Question: "Is this for an existing provider's catalogue file?"
- Options:
  - "Yes" (description: "Add product(s) to an existing catalogue feature file")
  - "No" (description: "Create a new catalogue feature file")

**Existing catalogue providers**: NOS (`NOS/NOS_Catalogue.feature`)

**If existing:** read the feature file, note the supplier name from `Given path 'catalogue/supplier', '...'`, note existing products in `providerExpected`. Skip to Catalogue Step 3.

**If new:** ask for the provider name and proceed through all steps.

---

### Catalogue Step 2: Supplier Name (new providers only)

Ask using AskUserQuestion:

**Question** (header: "Supplier Name")
- Question: "What is the exact supplier name as registered in the UAP catalogue? (e.g., 'NOS Comunicacoes S.A.' — no special characters, must match exactly)"
- Let the user type the exact string.

---

### Catalogue Step 3: Products to Add

For each product to validate, collect:

**Question** (header: "Supplier Product ID")
- Question: "What is the `supplierProductId`? (SCREAMING_SNAKE_CASE, e.g., `NOS_COMUNICACOES_EPL_EVPL_CARRIER_ETHERNET_SERVICES`)"

**Question** (header: "Product Name")
- Question: "What is the display name? (e.g., 'EPL/EVPL Carrier Ethernet Services')"

**Question** (header: "Requirement Type")
- Question: "What is the requirement type?"
- Options: `IP_SERVICE`, `E_ACCESS`, `BROADBAND`, `E_LINE`

**Question** (header: "Country Codes")
- Question: "Which country codes does this product cover? (comma-separated ISO3, e.g., `PRT`, `GBR,DEU`)"

**Question** (header: "Contract Terms")
- Question: "Which contract term lengths in months? (comma-separated, e.g., `12,24,36`)"

**Question** (header: "Access Media")
- Question: "Which access media types are supported? (comma-separated)"
- Options: `FIBRE`, `COPPER`, `WIRELESS`, `COAX`

**Question** (header: "Speeds")
- Question: "What download/upload speeds are available? Provide as Mbps pairs (e.g., `100/100, 200/200, 500/50`). Asymmetric pairs use `download/upload` format."
- Convert to kbps internally: multiply each value by 1000. Example: `100M` → `100000` kbps.

After collecting all products, ask:

**Question** (header: "Add More Products")
- Question: "Add another product to this catalogue?"
- Options: "Yes" / "No"

---

### Catalogue Step 4: Generate / Update the Catalogue Feature File

#### If adding to an existing provider:

1. Read the feature file
2. Add each new product entry to the `providerExpected` JSON object in Background
3. Add a `| NEW_PRODUCT_ID |` row to the Examples table
4. Match the existing formatting

#### If creating a new provider:

Create folder and file: `src/test/resources/features/provider/{ProviderFolder}/{Provider}_Catalogue.feature`

Use this template:

```gherkin
Feature: {Provider} catalogue validation
  Validates the {Provider} supplier product catalogue against the expected specification.
  See Confluence "{Provider} UAP Implementation" for expected configuration.
  Catalogue endpoint: GET /catalogue/supplier/{supplierName}
  Supplier ID: {Supplier Name as registered in UAP}

  Background:
    * url catalogueServiceUrl

    # Expected catalogue per Confluence specification.
    # Speeds expressed as { dl, ul } in kbps. Media names match catalogue enum values.
    * def {providerVar}Expected =
      """
      {
        "{SUPPLIER_PRODUCT_ID}": {
          "productName": "{Product Name}",
          "requirementType": "{IP_SERVICE | E_ACCESS | BROADBAND | E_LINE}",
          "countryCodes": ["{ISO3}"],
          "terms": [12, 24, 36],
          "media": ["{FIBRE | COPPER | WIRELESS | COAX}"],
          "speeds": [
            { "dl": 100000, "ul": 100000 }
          ]
        }
    ...
    Given path 'catalogue/supplier', '{Supplier Name as registered in UAP}'
    When method GET
    Then status 200
    * match response.supplierName == '{Supplier Name as registered in UAP}'
    * def {providerVar}Products = response.products

  # ============================================================================
  # Exact product list — no missing, no unexpected extras
  # ============================================================================

  @{provider_tag} @catalogue
  Scenario: {Provider} catalogue - registered products match specification
    * def expectedIds = Object.keys({providerVar}Expected)
    * def actualIds = {providerVar}Products.map(function(p) { return p.supplierProductId })
    * match actualIds contains only expectedIds

  # ============================================================================
  # Per-product detail validation via shared template
  # Validates: name, requirement type, country coverage, terms, media, speeds
  # All checks are bidirectional — missing AND unexpected extras both fail
  # ============================================================================

  @{provider_tag} @catalogue
  Scenario Outline: {Provider} catalogue - <supplierProductId>
    * def expected = {providerVar}Expected['<supplierProductId>']
    * def matches = {providerVar}Products.filter(function(p) { return p.supplierProductId === '<supplierProductId>' })
    * match matches == '#[1]'
    * def product = matches[0]
    * call read('classpath:helpers/catalogue_validation_template.feature') { product: '#(product)', expected: '#(expected)' }
...
**Template notes:**
- `{providerVar}` — camelCase variable name (e.g., `nos`, `esun`)
- Speeds in the expected JSON are **kbps** (multiply Mbps × 1000)
- `providerVar` is derived from provider tag: lowercase, no special characters (e.g., `nos`, `lumen`)

---

### Catalogue Step 5: Run Tests

**Question** (header: "Run Tests")
- Question: "Would you like to run the catalogue tests now?"
- Options: "Yes" / "No"
...
```bash
# Stage
./gradlew test '-Dkarate.options=--tags @{provider_tag} --tags @catalogue --tags ~@bug --tags ~@uat --tags ~@prod' -Dkarate.env=stage

# UAT
./gradlew test '-Dkarate.options=--tags @{provider_tag} --tags @catalogue --tags ~@bug --tags ~@stage --tags ~@prod' -Dkarate.env=uat

# Prod
./gradlew test '-Dkarate.options=--tags @{provider_tag} --tags @catalogue --tags ~@bug --tags ~@stage --tags ~@uat' -Dkarate.env=prod
```

---

### Catalogue Important Notes

- Speeds in the feature file are in **kbps** — always convert from Mbps: `100 Mbps = 100000 kbps`
- Supplier name in `Given path 'catalogue/supplier', '...'` must match exactly the name in UAP (no special characters like accents)
- All assertions are bidirectional — missing products/speeds AND unexpected extras both fail
- The `catalogueServiceUrl` Karate variable is pre-configured in `karate-config.js` per environment; no extra config needed
- `requirementType` values: `IP_SERVICE` (DIA), `E_ACCESS` (Ethernet Switched), `BROADBAND`, `E_LINE` (P2P)